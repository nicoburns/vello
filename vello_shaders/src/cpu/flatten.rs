...
trait VecExt {
    fn op(&self, other: Self) -> f32;
}
impl VecExt for Vec2 {
    fn op(&self, tangent: Self) -> f32 {
            ...
            if is_stroke {
                ...
                if is_stroke_cap_marker {
                    if is_open {
                        ...
                        let embolden = embolden.op(tangent);
                    ...
                    // Compute offset at start of segment
                    let start_embolden = embolden.op(tan_start);
                    let start_offset = 0.5 * (linewidth + start_embolden);
                    let n_start = start_offset * Vec2::new(-tan_start.y, tan_start.x).normalize();

                    // Compute offset at end of segment (at pts.p3)
                    let prev_embolden = embolden.op(tan_prev);
                    ...
                    // Compute offset for next segment (for the join)
                    let next_embolden = embolden.op(tan_next);
                ...
                } else if embolden.x == 0. && embolden_y == 0. {
                    flatten_euler(
                        &pts,
                        path_ix,
                        &transform,
